...
class Array
  def my_uniq
    arr_uniq = []
    self.each {|el| arr_uniq << el unless arr_uniq.include?(el) }
    arr_uniq
  end
end
  ...
  def two_sum
    ...
    for i in 0...self.length-1
      for j in i+1...self.length
        resulting_array << [i,j] if (self[i] + self[j] == 0)
      end
    end
    resulting_array
  end
...
class Array
  def median
    ...
    if self.length.even?
      median_el_1 = sorted_arr[self.length/2]
      median_el_2 = sorted_arr[self.length/2 - 1]
      median = (median_el_1 + median_el_2) / 2.0
    else
      median = sorted_arr[self.length/2]
    end
...
class Array
  def my_transpose
    transp_array = []
    for row in 0...self[0].length
      temp_array= []
      for col in 0...self.length
        temp_array << self[col][row]
      end
      transp_array << temp_array
    end
    transp_array
  end
end
